...
function addNotification(allRequestNotification) {
  $("#listNotifications").append(
    `<li id="noti` + allRequestNotification.userId + `" class="noti"><img src="` +
...
`</p>
<button onclick="acceptRequest(` + allRequestNotification.userId + `)" id="addRequest` + allRequestNotification.userId + `">
Accept</button>
<button onclick="rejectRequest(` + allRequestNotification.userId + `)" id="rejectRequest` + allRequestNotification.userId + `" >
Reject</button></li>`
);
}
...
function showAllSearch(user){
 ...
 user.lastname +
 `</p>
 <button onclick="sendRequest(` + user.id + `)" id="friendRequest` + user.id + `" style="display:` + disagain + `">
 Send Request</button>
 <button onclick="cancleRequest(` + user.id + `)" id="cancleFriendRequest` + user.id + `" style="display:` + dis + `">
 Cancle Request</button>
 <button onclick="acceptRequest(` + user.requestId + `)" id="addRequest` + user.requestId + `" style="display:` + accept + `" >
Accept</button>
<button onclick="rejectRequest(` + user.requestId + `)" id="rejectRequest` + user.requestId + `" style="display:` + reject + `" >
Reject</button>
<button onclick="sendMessage(` + user.requestId + `)" id="sendMess` + user.requestId + `" style="display:` + mess + `" >
Message</button></li>`
  );
}
...
$(document).ready(function () {
  ...
  function addpost(post) {
    ...
    <h3>` + 
        post.projectName +
        `</h3>
        <p class='teamname'> by ` + post.teamName + `
        </p>
    <div class="dropdown">
    ...
    <a href= "` + post.projectDocuments + `" class="viewdocs" id="viewdocs` + post.id + 
